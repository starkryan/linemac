...
import DirectRDService from "@/components/mantra/DirectRDService";
import { MantraCaptureSection } from "@/components/mantra/MantraCaptureSection";
import { MorphoConfigChecker } from "@/components/mantra/MorphoConfigChecker";
...
export default function GovernmentForm() {
  ...
  const [rdServiceResponse, setRdServiceResponse] = useState<any>(null);
  const [mantraCaptureData, setMantraCaptureData] = useState<any>(null);
  const [morphoConfig, setMorphoConfig] = useState<any>(null);
        ...
        if (response.ok) {
          ...
          if (sessionData?.user) {
            ...
            setFormData((prev) => ({
              ...
              mobile_number:
                prev.mobile_number || sessionData.user.mobile || "",
            }));
          }
        }
      } catch (error) {
  ...
  const validateAndSubmit = async () => {
    ...
    if (FormValidation.hasErrors(errors)) {
      ...
      showModal(
        "error",
        "Validation Error",
        FormValidation.formatError(
          Object.keys(errors)[0],
          firstError || "Please check your form"
        )
      );
      return;
    }
    ...
    if (!confirmationChecked) {
      showModal(
        ...
        "Please confirm that all information provided is accurate before submitting."
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        showModal(
          ...
          "Your Aadhaar correction application has been submitted successfully!",
          () => {
  ...
  const handleRDServiceResponse = (response: any) => {
    setRdServiceResponse(response);
    console.log("RDService Response:", response);

    // If capture was successful, update fingerprint data
    if (
      response.ok &&
      response.method === "CAPTURE" &&
      response.result?.parsed?.pidData
    ) {
      const pidData = response.result.parsed.pidData;
      if (pidData.resp && pidData.resp.errCode === "0") {
        const captureData: CaptureResponse = {
          pidData: pidData.data || "",
          score: parseInt(pidData.resp.qScore) || 0,
          status: "success",
        };

        // Determine capture type (default to left hand, can be enhanced)
        const captureType = "left"; // This could be made configurable
        setFingerprintData((prev) => ({
          ...prev,
          [captureType]: captureData,
        }));
      }
    }
  };

  const handleMantraCaptureComplete = (data: any) => {
    setMantraCaptureData(data);
    console.log("Mantra Capture Data:", data);

    // Also update fingerprint data for compatibility
    if (data.errCode === "0") {
      const captureData: CaptureResponse = {
        pidData: data.captureData?.data || "",
        score: parseInt(data.qScore) || 0,
        status: "success",
      };
  ...
  const handleMorphoConfigCheck = (config: any) => {
    setMorphoConfig(config);
    console.log("Morpho Configuration:", config);
  };

  // File handler functions
  const handleFileSelect = (
    fileType: "dobProof" | "identityProof" | "addressProof" | "porDocument",
    file: File
  ) => {
        ...
        <div className="p-6 max-w-7xl mx-auto">
                ...
                <div className="bg-white p-4">
                  <div className="flex items-center gap-8">
                    ...
                    <RadioGroup
                      ...
                      onValueChange={(value) =>
                        handleInputChange("updateType", value)
                      }
                      className="flex items-center gap-6"
                    >
                      <div className="flex items-center space-x-2">
                  ...
                  <div className="mt-3 text-sm text-gray-600">
                    Select the type of update you want to perform. The form will
                    show relevant sections based on your selection.
                  </div>
                </div>
              </div>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleInputChange("name_hindi", e.target.value)
                          }
                          ...
                          <div className="flex items-center space-x-2">
                            <Checkbox
                              ...
                              onCheckedChange={(checked) =>
                                setIsDOBVerified(checked as boolean)
                              }
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleInputChange("npr_receipt", e.target.value)
                          }
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div>
                        ...
                        <Input
                          ...
                          value={formData.co}
                          onChange={(e) =>
                            handleInputChange("co", e.target.value)
                          }
                          placeholder="Care of"
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleInputChange("co_hindi", e.target.value)
                          }
                          placeholder="द्वारा"
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleInputChange("house_no", e.target.value)
                          }
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleInputChange("house_no_hindi", e.target.value)
                          }
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleInputChange("street_hindi", e.target.value)
                          }
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleInputChange("landmark_hindi", e.target.value)
                          }
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleInputChange("area_hindi", e.target.value)
                          }
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleInputChange("city_hindi", e.target.value)
                          }
                ...
                <div className="bg-white p-4">
                  <div className="space-y-4">
                    ...
                    <div className="flex items-start gap-4">
                      <Label className="text-sm text-gray-700 w-20 pt-2">
                        Scanner
                      </Label>
              ...
              <div className="mb-6">
                ...
                <div className="bg-white p-4">
                  <div className="flex items-center gap-4">
                    ...
                    <FileUpload
                      onFileSelect={(file) =>
                        handleFileSelect("dobProof", file)
                      }
                      ...
                      {verificationMethod === "documents" && (
                        <>
                          <div className="flex items-center gap-4 ml-6">
                            ...
                            <FileUpload
                              onFileSelect={(file) =>
                                handleFileSelect("identityProof", file)
                              }
                              onFileView={() => handleFileView("identityProof")}
                              onFileRemove={() =>
                                handleFileRemove("identityProof")
                              }
                              acceptedTypes=".pdf,.jpg,.jpeg,.png"
                              maxSize={5}
                            />
                          ...
                          <div className="flex items-center gap-4 ml-6">
                            ...
                            <FileUpload
                              onFileSelect={(file) =>
                                handleFileSelect("addressProof", file)
                              }
                              onFileView={() => handleFileView("addressProof")}
                              onFileRemove={() =>
                                handleFileRemove("addressProof")
                              }
                              acceptedTypes=".pdf,.jpg,.jpeg,.png"
                              maxSize={5}
                            />
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <Label
                          ...
                          className="text-sm text-gray-700"
                        >
                          Head of Family shall verify the resident's
                          identity/address.
                        </Label>
                      </div>
                      ...
                      {verificationMethod === "family-head" && (
                        <div className="flex items-center gap-4 ml-6">
                          ...
                          <Select>
                            ...
                            <SelectContent>
                              <SelectItem value="por1">
                                POR Document 1
                              </SelectItem>
                              <SelectItem value="por2">
                                POR Document 2
                              </SelectItem>
                            </SelectContent>
                          </Select>
                          <FileUpload
                            onFileSelect={(file) =>
                              handleFileSelect("porDocument", file)
                            }
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setFormData((prev) => ({
                              ...prev,
                              head_of_family_name: e.target.value,
                            }))
                          }
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setFormData((prev) => ({
                              ...prev,
                              head_of_family_name_hindi: e.target.value,
                            }))
                          }
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setFormData((prev) => ({
                              ...prev,
                              relative_aadhaar: e.target.value,
                            }))
                          }
                        />
                      </div>
          ...
          {activeTab === "fingerprints" && (
            <div className="space-y-6">
              {/* Direct RDService Interface
            <div className="mb-6">
              <div className="bg-gray-200 px-4 py-2 border border-gray-300">
                <h2 className="text-base font-semibold text-gray-800">Mantra RDService Direct Interface</h2>
              </div>
              <div className="bg-white p-4">
                <DirectRDService onResponse={handleRDServiceResponse} />
              </div>
            </div> */}

              {/* RDService Response Display */}
              {rdServiceResponse && (
                <div className="mb-6">
                  <div className="bg-gray-200 px-4 py-2 border border-gray-300">
                    <h2 className="text-base font-semibold text-gray-800">
                  ...
                  <div className="bg-white p-4">
                    <div className="text-sm space-y-2">
                      <div>
                        ...
                        {rdServiceResponse.ok ? "Success" : "Failed"}
                      </div>
                      <div>
                        <strong>Method:</strong> {rdServiceResponse.method}
                      </div>
                      {rdServiceResponse.message && (
                        <div>
                          <strong>Message:</strong> {rdServiceResponse.message}
                        </div>
                      )}
                      {rdServiceResponse.result?.parsed && (
                        <div className="mt-3">
              ...
              {/* Biometric Section - Alternative Interface */}
              <div className="mb-6">
                <BiometricSection
                  mode="fingerprints"
                  deviceType="both"
            ...
            <div className="mb-6">
              <div className="bg-gray-200 px-4 py-2 border border-gray-300">
                <h2 className="text-base font-semibold text-gray-800">Quick Fingerprint Capture</h2>
              </div>
              <div className="bg-white p-4">
                <MantraCaptureSection onCaptureComplete={handleMantraCaptureComplete} />
              </div>
            </div> */}
            ...
            {mantraCaptureData && (
              <div className="mb-6">
                <div className="bg-gray-200 px-4 py-2 border border-gray-300">
                  <h2 className="text-base font-semibold text-gray-800">Capture Results</h2>
                </div>
                <div className="bg-white p-4">
                  <div className="text-sm space-y-2">
                    <div><strong>Error Code:</strong> {mantraCaptureData.errCode}</div>
                    <div><strong>Error Info:</strong> {mantraCaptureData.errInfo}</div>
                    <div><strong>Quality Score:</strong> {mantraCaptureData.qScore}%</div>
                    <div><strong>Minutiae Points:</strong> {mantraCaptureData.nmPoints}</div>
                    <div className="mt-3">
                      <strong>Success Rate:</strong>
                      <span className={`ml-2 px-2 py-1 rounded text-xs font-medium ${
                        parseInt(mantraCaptureData.qScore) >= 90 ? 'bg-green-100 text-green-800' :
                        parseInt(mantraCaptureData.qScore) >= 70 ? 'bg-blue-100 text-blue-800' :
                        parseInt(mantraCaptureData.qScore) >= 50 ? 'bg-yellow-100 text-yellow-800' :
                        'bg-red-100 text-red-800'
                      }`}>
                        {parseInt(mantraCaptureData.qScore) >= 90 ? 'Excellent' :
                         parseInt(mantraCaptureData.qScore) >= 70 ? 'Good' :
                         parseInt(mantraCaptureData.qScore) >= 50 ? 'Fair' : 'Poor'}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              {/* Morpho Configuration Results Display */}
              {morphoConfig && (
                <div className="mb-6">
                  <div className="bg-gray-200 px-4 py-2 border border-gray-300">
                    <h2 className="text-base font-semibold text-gray-800">
                  ...
                  <div className="bg-white p-4">
                    <div className="text-sm space-y-2">
                      <div>
                        <strong>RDService Status:</strong>{" "}
                        {morphoConfig.rdService ? "Connected" : "Not Found"}
                      </div>
                      <div>
                        <strong>Device Status:</strong>{" "}
                        {morphoConfig.deviceInfo ? "Detected" : "Not Detected"}
                      </div>
                      <div>
                        <strong>Working Port:</strong>{" "}
                        {morphoConfig.workingPort || "N/A"}
                      </div>
                      {morphoConfig.rdService && (
                        <div className="mt-3">
                          <strong>RDService Details:</strong>
                          <div className="ml-4 text-xs text-gray-600">
                            {Object.entries(morphoConfig.rdService).map(
                              ([key, value]) => (
                                <div key={key}>
                                  {key}: {String(value)}
                                </div>
                      ...
                      {morphoConfig.deviceInfo && (
                        <div className="mt-3">
                          <strong>Device Details:</strong>
                          <div className="ml-4 text-xs text-gray-600">
                            {Object.entries(morphoConfig.deviceInfo).map(
                              ([key, value]) => (
                                <div key={key}>
                                  {key}: {String(value)}
                                </div>
            ...
            <>
              ...
              <div className="mb-6">
                ...
                <div className="bg-white p-4">
                  <div className="text-sm text-gray-600 mb-4">
                    Please review all the information entered before final
                    submission. Click on any section to edit.
                  </div>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-6 text-sm">
                    <div>
                      ...
                      <div className="mb-2">
                        ...
                        {formData.aadhaar_number || "[To be filled]"}
                      </div>
                      <div className="mb-2">
                        <strong>Name:</strong>{" "}
                        {formData.name || "[To be filled]"}
                      </div>
                      <div className="mb-2">
                        <strong>Gender:</strong>{" "}
                        {formData.gender || "[To be selected]"}
                      </div>
                      <div className="mb-2">
                        <strong>Age/DOB:</strong>{" "}
                        {formData.dob || "[To be filled]"}{" "}
                        {isDOBVerified && (
                      ...
                      <div className="mb-2">
                        <strong>NPR Receipt/TIN No.:</strong>{" "}
                        {formData.npr_receipt || "[To be filled]"}
                      </div>
                    </div>
                    <div>
                      <div className="mb-2">
                      ...
                      <div className="mb-2">
                        ...
                        {formData.aadhaar_number || "[भरा जाना है]"}
                      </div>
                      <div className="mb-2">
                        <strong>नाम:</strong>{" "}
                        {formData.name_hindi || "[भरा जाना है]"}
                      </div>
                      <div className="mb-2">
                        <strong>लिंग:</strong>{" "}
                        {formData.gender
                      ...
                      <div className="mb-2">
                        <strong>आयु या जन्म तिथि:</strong>{" "}
                        {formData.dob || "[भरा जाना है]"}
                      </div>
                      <div className="mb-2">
                        <strong>गांव/कस्बा/शहर:</strong>{" "}
                        {formData.city_hindi || "[भरा जाना है]"}
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-6 text-sm">
                    <div>
                      ...
                      <div className="mb-2">
                        ...
                        {formData.house_no || "[To be filled]"}
                      </div>
                      <div className="mb-2">
                        <strong>Street/Road/Lane:</strong>{" "}
                        {formData.street || "[To be filled]"}
                      </div>
                      <div className="mb-2">
                        <strong>Landmark:</strong>{" "}
                        {formData.landmark || "[To be filled]"}
                      </div>
                      <div className="mb-2">
                        <strong>Area/Locality/Sector:</strong>{" "}
                        {formData.area || "[To be filled]"}
                      </div>
                      <div className="mb-2">
                        <strong>Village/Town/City:</strong>{" "}
                        {formData.city || "[To be filled]"}
                      </div>
                    </div>
                    <div>
                      <div className="mb-2">
                        <strong>द्वारा:</strong>{" "}
                        {formData.co_hindi || "[भरा जाना है]"}
                      </div>
                      <div className="mb-2">
                        <strong>घर/निर्माण:</strong>{" "}
                        {formData.house_no_hindi || "[भरा जाना है]"}
                      </div>
                      <div className="mb-2">
                        <strong>सड़क/मार्ग/गली:</strong>{" "}
                        {formData.street_hindi || "[भरा जाना है]"}
                      </div>
                      <div className="mb-2">
                        <strong>स्थान चिह्न:</strong>{" "}
                        {formData.landmark_hindi || "[भरा जाना है]"}
                      </div>
                      <div className="mb-2">
                        <strong>स्थान:</strong>{" "}
                        {formData.area_hindi || "[भरा जाना है]"}
                      </div>
                      <div className="mb-2">
                        <strong>गांव/कस्बा/शहर:</strong>{" "}
                        {formData.city_hindi || "[भरा जाना है]"}
              ...
              <div className="mb-6">
                ...
                <div className="bg-white p-4">
                  <div className="text-sm space-y-2">
                    <div>
                      <strong>Scanner Configuration:</strong>{" "}
                      {selectedScanner === "wia-brother"
                        ? "WIA-Brother Scanner c1 #2"
                        : selectedScanner}{" "}
                      (Color Mode)
                    </div>
                    <div>
                      <strong>Date of Birth Proof:</strong>{" "}
                      {formData.dob_proof_type || "[To be selected]"}
                    </div>
                    <div>
                    ...
                    {verificationMethod === "documents" && (
                      <>
                        <div>
                          ...
                          {formData.identity_proof_type || "[To be selected]"}
                        </div>
                        <div>
                          <strong>Address Proof:</strong>{" "}
                          {formData.address_proof_type || "[To be selected]"}
                        </div>
                      </>
                    )}
                    {verificationMethod === "family-head" && (
                      <div>
                        <strong>POR Document:</strong>{" "}
                        {formData.por_document_type || "[To be selected]"}
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="mb-6">
                ...
                <div className="bg-white p-4">
                  <div className="text-sm space-y-2">
                    <div>
                      <strong>Main Photograph:</strong>{" "}
                      {photoCounts.main === 4
                        ? "Not captured"
                        : `${4 - photoCounts.main} photo(s) captured`}{" "}
                      ({photoCounts.main} remaining)
                    </div>
                    <div>
                      <strong>Exception Photograph:</strong>{" "}
                      {photoCounts.exception === 4
                        ? "Not captured"
                        : `${4 - photoCounts.exception} photo(s) captured`}{" "}
                      ({photoCounts.exception} remaining)
                    </div>
                    <div>
                      <strong>Status:</strong>{" "}
                      {photoCounts.main === 4 && photoCounts.exception === 4
                        ? "Pending"
                        : "In Progress"}
                    </div>
                  </div>
              ...
              <div className="mb-6">
                ...
                <div className="bg-white p-4">
                  <div className="text-sm space-y-2">
                    <div>
                      <strong>Left Hand:</strong> Not captured (4 remaining) -
                      Score: 0%
                    </div>
                    <div>
                      <strong>Right Hand:</strong> Not captured (4 remaining) -
                      Score: 0%
                    </div>
                    <div>
                      <strong>Both Thumbs:</strong> Not captured (4 remaining) -
                      Score: 0%
                    </div>
                  </div>
                </div>
              ...
              <div className="mb-6">
                ...
                <div className="bg-white p-4">
                  <div className="text-sm text-gray-600 mb-4">
                    By clicking "Submit Application", you confirm that all the
                  ...
                  <div className="flex items-center space-x-2 mb-4">
                    ...
                    <Label htmlFor="confirm" className="text-sm text-gray-700">
                      ...
                      understand that providing false information is a
                      punishable offense.
                    </Label>
                  </div>
                  <Button
